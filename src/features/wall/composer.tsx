        ...
        {step === 2 ? (
          <div className="composer-body details-step">
            ...
            <div className="form-grid">
              ...
              <label className={!form.category ? "field-locked" : ""}>Subcategory<select required value={form.subcategory} disabled={!form.category} onChange={(event) => setForm((value) => ({ ...value, subcategory: event.target.value }))}><option value="" disabled>{form.category ? "— Select a type —" : "Pick a category first"}</option>{form.category ? SUBCATEGORY_OPTIONS[form.category as CardCategory].map((sub) => <option key={sub} value={sub}>{sub}</option>) : null}</select>{!form.category ? <small className="field-help">Choose a category before picking a subcategory.</small> : null}</label>
            </div>
            <div className="form-grid">
